...
class Base_Task(gym.Env):
    ...
    def take_action(self, action, action_type:Literal['qpos', 'ee']='qpos'):  # action_type: qpos or ee
        ...
        if (self.eval_video_path is not None and self.take_action_cnt % eval_video_freq == 0):
            # `now_obs` is normally refreshed only when the policy replans.
            # Using it here made every video frame within an open-loop chunk
            # stale, with an artificial jump at the following replan boundary.
            # Video is diagnostic-only: capture a fresh pre-action observation
            # without changing the observation passed to the policy.
            video_obs = self.get_obs()
            self.eval_video_ffmpeg.stdin.write(
                video_obs["observation"]["head_camera"]["rgb"].tobytes()
            )
        ...
        # Task-agnostic execution telemetry.  It is reset by the evaluator at
        # the start of each policy episode and is intentionally maintained in
        # the common action path so every RoboTwin task has comparable traces.
        self.last_take_action_trace = {
            "terminal_before_action": False,
            "topp_left_success": bool(topp_left_flag),
            "topp_right_success": bool(topp_right_flag),
            "left_control_steps": int(left_n_step),
            "right_control_steps": int(right_n_step),
            "control_scene_steps": 0,
            "terminal_success_after_action": False,
            "action_limit_reached": False,
        }
        if topp_left_flag and topp_right_flag:
            self.effective_policy_actions = getattr(self, "effective_policy_actions", 0) + 1
        ...
        while now_left_id < left_n_step or now_right_id < right_n_step:
            ...
            self.scene.step()
            self.policy_scene_steps = getattr(self, "policy_scene_steps", 0) + 1
            self.last_take_action_trace["control_scene_steps"] += 1
            self._update_render()

            if self.check_success():
                self.eval_success = True
                self.last_take_action_trace["terminal_success_after_action"] = True
        ...
        self._update_render()
        self.last_take_action_trace["action_limit_reached"] = self.take_action_cnt >= self.step_lim
